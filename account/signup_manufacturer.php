...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($firstname) || empty($email) || empty($password) || empty($repeat_password)) {
        ...
        $role="contractor";
        $stmt->bind_param("ssss", $firstname, $email, $hashed_password,$role);

        // Execute the statement
        if ($stmt->execute()) {
            header("Location: session.php?id=$user_id&name=" . urlencode($firstname) . "&role=contractor");
            exit;
        } else {
...
<body>
  <div class="wrapper">
    ...
    <p>Already have an Account? <a href="login_form_manufacturer.php">login</a></p>
  </div>
</body>
</html>
